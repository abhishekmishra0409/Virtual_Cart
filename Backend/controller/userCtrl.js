...
const saveAddress = asyncHandler(async (req, res, next) => {
    ...
    if (!street || !city || !state || !pincode || !country) {
        res.status(400);
        throw new Error("All address fields (street, city, state, pincode, and country) are required.");
    }
    try {
        const updatedUser = await User.findByIdAndUpdate(
            _id,
...
const createOrder = asyncHandler(async (req, res) => {
    ...
    try {
        if (!COD) {
            return res.status(400).json({ message: "Cash on Delivery option is required" });
        }
        ...
        const detailedProducts = await Promise.all(userCart.products.map(async (item) => {
        ...
        await Cart.findOneAndUpdate({ userId: _id }, { products: [], cartTotal: 0, totalAfterDiscount: 0 });
