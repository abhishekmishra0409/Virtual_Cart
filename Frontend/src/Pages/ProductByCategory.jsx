...
import { fetchProductsByCategory} from "../features/Product/ProductSlice.js";
import {useParams} from "react-router-dom";

export const ProductByCategory = () => {
    const dispatch = useDispatch();
    const { id } = useParams();
    const { productsByCategory, isLoading, isError } = useSelector((state) => state.products);
    ...
    if (isError) return <div>Error fetching products By Category</div>;

    // Pagination logic
    const indexOfLastProduct = currentPage * productsPerPage;
    const indexOfFirstProduct = indexOfLastProduct - productsPerPage;
    const currentProducts = productsByCategory.slice(indexOfFirstProduct, indexOfLastProduct);
    const totalPages = Math.ceil(productsByCategory.length / productsPerPage);
                                ...
                                <div className="row">
                                    ...
                                    <div className="col-xl-10 col-lg-9 mb-10 text-lg-end text-center d-flex justify-content-end align-items-center">
                                        <span className="font-sm color-gray-900 font-medium me-3">
                                            Showing {indexOfFirstProduct + 1}–{Math.min(indexOfLastProduct, productsByCategory.length)} of {productsByCategory.length} results
                                        </span>
                                        ...
                                        {/*            id="dropdownSort"*/}
                                        {/*        >*/}
                                        {/*            <span className="font-sm color-gray-900 font-medium me-2">Latest productsByCategory</span>*/}
                                        {/*        </Dropdown.Toggle>*/}
                                        {/*        <Dropdown.Menu>*/}
                                        {/*            <Dropdown.Item href="#/action-1" active>Latest productsByCategory</Dropdown.Item>*/}
                                        {/*            <Dropdown.Item href="#/action-2">Oldest productsByCategory</Dropdown.Item>*/}
                                        {/*            <Dropdown.Item href="#/action-3">Top-rated productsByCategory</Dropdown.Item>*/}
                                        {/*        </Dropdown.Menu>*/}
                                        {/*    </Dropdown>*/}
                                        {/*</div>*/}
