...
import { fetchProductsByPrice} from "../features/Product/ProductSlice.js";
import {useParams} from "react-router-dom";

export const ProductByPrice = () => {
    const dispatch = useDispatch();
    const { gte,lte } = useParams();
    const { productsByPrice, isLoading, isError } = useSelector((state) => state.products);
    ...
    if (isError) return <div>Error fetching products By Price</div>;

    // Pagination logic
    const indexOfLastProduct = currentPage * productsPerPage;
    const indexOfFirstProduct = indexOfLastProduct - productsPerPage;
    const currentProducts = productsByPrice.slice(indexOfFirstProduct, indexOfLastProduct);
    const totalPages = Math.ceil(productsByPrice.length / productsPerPage);
                                ...
                                <div className="row">
                                    ...
                                    <div className="col-xl-10 col-lg-9 mb-10 text-lg-end text-center d-flex justify-content-end align-items-center">
                                        <span className="font-sm color-gray-900 font-medium me-3">
                                            Showing {indexOfFirstProduct + 1}–{Math.min(indexOfLastProduct, productsByPrice.length)} of {productsByPrice.length} results
                                        </span>
                                        ...
                                        {/*            id="dropdownSort"*/}
                                        {/*        >*/}
                                        {/*            <span className="font-sm color-gray-900 font-medium me-2">Latest productsByPrice</span>*/}
                                        {/*        </Dropdown.Toggle>*/}
                                        {/*        <Dropdown.Menu>*/}
                                        {/*            <Dropdown.Item href="#/action-1" active>Latest productsByPrice</Dropdown.Item>*/}
                                        {/*            <Dropdown.Item href="#/action-2">Oldest productsByPrice</Dropdown.Item>*/}
                                        {/*            <Dropdown.Item href="#/action-3">Top-rated productsByPrice</Dropdown.Item>*/}
                                        {/*        </Dropdown.Menu>*/}
                                        {/*    </Dropdown>*/}
                                        {/*</div>*/}
